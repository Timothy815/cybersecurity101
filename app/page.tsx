...
const firstDispatchPath = "/dispatch/the-machine-that-can-answer-anything";
...
export default function Home(){
  ...
  const results=useMemo(()=>{
    ...
    return [["Dispatch","The Machine That Can Answer Anything","Responsibility, artificial intelligence, and the future cyber defender.",firstDispatchPath],...resources.map(r=>["Resource",r[0],r[1],"#resources"]),...readings.map(r=>["Reading",r[0],r[1],"#readings"]),...terms.map(t=>["Word wall",t[0],t[1],"#word-wall"])].filter(r=>r.join(" ").toLowerCase().includes(q)).slice(0,6);
  },[query]);
  return <main id="top">
    <header><a className="brand" href="#top"><Logo/><span><strong>CYBER / CLASSROOM</strong><small>Student operations hub</small></span></a><nav aria-label="Primary navigation"><a href="#dispatch">Dispatch</a><a href="#resources">Resources</a><a href="#readings">Readings</a><a href="#word-wall">Word wall</a></nav><a className="portal" href="https://classroom.google.com/h/tv" target="_blank" rel="noreferrer">Class portal ↗</a></header>
    <section className="hero">
      <div className="hero-copy"><div className="online"><i/> Systems online · Classroom hub</div><p className="label">Cybersecurity operations // 2026–27</p><h1>Learn to think<br/>like a <em>defender.</em></h1><p className="lead">Your launch point for class updates, Security+ preparation, readings, technical resources, and the language of cybersecurity.</p><div className="actions"><Link className="button" href={firstDispatchPath}>Read the latest dispatch <b>→</b></Link><a href="#resources">Browse resource vault ↘</a></div>
      ...
      <aside className="brief"><div className="brief-top"><span>Current briefing</span><b>ED. 01</b></div><div className="brief-body"><small>August // Opening brief</small><h2>The Machine That Can Answer Anything</h2><p>What responsible AI use asks of the future cyber defender—and why judgment must remain human.</p></div><dl><div><dt>Focus</dt><dd>AI responsibility</dd></div><div><dt>Read</dt><dd>14 minutes</dd></div><div><dt>Status</dt><dd className="ready">● Ready</dd></div></dl><Link href={firstDispatchPath}>Open full dispatch <span>→</span></Link></aside>
    </section>
    ...
    <section className="section dispatch" id="dispatch"><div className="section-head"><div><p className="label">01 // Classroom dispatch</p><h2>What matters <em>right now.</em></h2></div><p>One concise briefing for announcements, learning priorities, deadlines, and what is coming next.</p></div><div className="dispatch-grid">
      <article className="dispatch-main"><div className="meta"><span>Latest edition</span><span>August 2026</span><span>14 min read</span></div><h3>The Machine That Can Answer Anything</h3><p>AI can investigate alerts, generate code, and explain vulnerabilities at extraordinary speed. This opening dispatch asks what happens when a machine sounds certain, gets something wrong, and leaves a human being to carry the consequences.</p><div className="steps"><div><b>01</b><p><strong>Question</strong>Treat confident output as a lead, not a verdict.</p></div><div><b>02</b><p><strong>Protect</strong>Decide what information should never enter a prompt.</p></div><div><b>03</b><p><strong>Own</strong>Explain your reasoning and accept responsibility for the result.</p></div></div><Link className="dark-button" href={firstDispatchPath}>Read full dispatch →</Link></article>
